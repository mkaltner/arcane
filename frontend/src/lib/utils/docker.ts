...
export type ThemedIconSource = {
	iconLightUrl?: string | null;
	iconDarkUrl?: string | null;
};

export function getThemedIconUrl(source?: ThemedIconSource | null, themeMode?: string | null): string | null {
	if (!source) return null;
	if (themeMode === 'light') {
		return source.iconDarkUrl?.trim() || null;
	}
	return source.iconLightUrl?.trim() || null;
}
